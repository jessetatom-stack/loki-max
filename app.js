...
function initPWA(){ if('serviceWorker' in navigator){ window.addEventListener('load', async ()=>{ try { const reg = await navigator.serviceWorker.register(`./sw.js?v=${window.__APP_BUILD__ || 'dev'}`); if (reg.update) reg.update(); } catch(e) { console.error(e); } }); } window.addEventListener('beforeinstallprompt',e=>{ e.preventDefault(); deferredPrompt=e; document.getElementById('installBtn').classList.remove('hidden'); }); document.getElementById('installBtn').onclick=async()=>{ if(!deferredPrompt) return toast('Use browser menu to install'); deferredPrompt.prompt(); await deferredPrompt.userChoice; deferredPrompt=null; document.getElementById('installBtn').classList.add('hidden'); }; }
...
(function forceVersionRefresh(){
  const build = window.__APP_BUILD__ || 'dev';
  const key = 'loki-max-build';
  const previous = localStorage.getItem(key);
  if(previous !== build){
    localStorage.setItem(key, build);
    if('serviceWorker' in navigator){
      navigator.serviceWorker.getRegistrations().then(regs => regs.forEach(r => r.update && r.update()));
    }
    setTimeout(() => { toast(`Updated to ${build}`); }, 300);
  }
})();
